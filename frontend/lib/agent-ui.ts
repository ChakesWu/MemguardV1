...
export function parseOutputEvidence(answer: string, report: unknown): OutputEvidenceLink[] {
  ...
  return links.flatMap(value => {
    ...
    if (!validOffset(start) || !validOffset(end) || start >= end || end > answer.length ||
      typeof segment !== 'string' || answer.slice(start, end) !== segment ||
      typeof memoryId !== 'string' || !memoryId || typeof quote !== 'string' || typeof role !== 'string' ||
      link?.prompt_included !== true || link?.validation_status !== 'valid' ||
      (typeof trustScore !== 'number' && trustScore !== null) || typeof policyAction !== 'string') return []
    const source = sources.get(memoryId)
    return [{
      startOffset: start, endOffset: end, segment, memoryId, evidenceQuote: quote, role,
      trustScore, trustLevel: typeof trust?.level === 'string' ? trust.level : 'unknown', policyAction,
...
export function outputEvidenceParts(answer: string, links: OutputEvidenceLink[]): OutputEvidencePart[] {
  ...
  const ranges = [...groups.values()].map(rangeLinks => ({ start: rangeLinks[0].startOffset, end: rangeLinks[0].endOffset, links: rangeLinks })).sort((left, right) => left.start - right.start || left.end - right.end)
