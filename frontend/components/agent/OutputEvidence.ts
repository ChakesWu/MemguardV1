...
export default function OutputEvidence({ answer, links }: OutputEvidenceProps) {
  ...
  return createElement('p', { className: 'mg-agent-answer' }, parts.map((part, index) => {
    ...
    return createElement('span', {
      ...
      createElement('button', {
        type: 'button',
        className: 'mg-evidence-chip',
        'aria-label': `Open evidence for ${part.text}`,
        key: 'chip',
      }, [
        ...
        createElement('span', { className: 'mg-evidence-card', role: 'tooltip', key: 'card' }, [
          ...
          createElement('span', { className: 'mg-evidence-card__link', key: 'link' }, 'Open full evidence ↗'),
        ]),
      ]),
    ])
  }))
