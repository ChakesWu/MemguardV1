...
type OutputEvidenceProps = { answer: string; links: OutputEvidenceLink[]; onOpenEvidence?: (link: OutputEvidenceLink) => void }
...
function cardRow(label: string, value: string) {
  return createElement('span', { className: 'mg-evidence-card__row', key: label }, [
    createElement('b', { key: 'label' }, label), createElement('em', { key: 'value' }, value),
  ])
}
...
export default function OutputEvidence({ answer, links, onOpenEvidence }: OutputEvidenceProps) {
  ...
  return createElement('p', { className: 'mg-agent-answer' }, parts.map((part, index) => {
    ...
    return createElement('span', { className: 'mg-evidence-anchor', key: `evidence-${primary.startOffset}-${primary.endOffset}` }, [
      ...
      createElement('span', { className: 'mg-evidence-card', role: 'group', 'aria-label': 'Memory evidence', key: 'card' }, [
        ...
        createElement('button', { type: 'button', className: 'mg-evidence-card__link', 'aria-haspopup': 'dialog', onClick: () => onOpenEvidence?.(primary), key: 'link' }, 'Open full evidence ↗'),
      ]),
    ])
  }))
}
