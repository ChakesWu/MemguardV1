...
def seed_baseline_data(repository: SupportRepository, tenant_id: str = "acme-dev") -> None:
    ...
    repository.upsert_order(Order(tenant_id, "ORD-4821", "CUS-1042", "Noise-cancelling headphones", "delivered", datetime(2026, 7, 5, 10, tzinfo=timezone.utc), "paid", {"city": "Hong Kong"}, source_type="support_order_db", source_id="ORD-4821", writer_id="support-order-sync", source_updated_at=now - timedelta(days=2), verified_at=now - timedelta(days=1), conflict_status="none"))
