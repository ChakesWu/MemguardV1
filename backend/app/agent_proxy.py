...
def extract_sse_payloads(buffer: bytes, chunk: bytes) -> tuple[bytes, list[dict[str, Any]]]:
    ...
    while b"\n\n" in buffer:
        ...
        data = b"\n".join(line[5:].lstrip() for line in raw_event.split(b"\n") if line.startswith(b"data:"))
        try:
...
def governed_output_records(
    *, tenant_id: str, agent_id: str, session_id: str, user_input: str,
    answer: str, report: dict[str, Any],
) -> tuple[list[MemoryEvent], DecisionTrace]:
    """Convert validated evidence into human-readable, auditable console records."""
    ...
    for link in valid_links:
        ...
        item_trust = item.get("trust", {}) if isinstance(item, dict) else {}
        content = item.get("content") if isinstance(item, dict) else None
        if not isinstance(memory_id, str) or not isinstance(trust.get("score"), (int, float)):
            continue
        if not isinstance(content, str) or content in {"[hash-only]", "[redacted]"}:
            content = str(link.get("evidence_quote") or "[content unavailable]")
        event = MemoryEvent(
            ...
            content=content, content_hash=str((item or {}).get("content_hash") or ""),
            policy_decision=str(policy.get("action") or "review_required"),
            trust_score=float(trust["score"]), created_at=datetime.now(timezone.utc).isoformat(),
            metadata={
                "evidence_role": link.get("role"), "evidence_quote": link.get("evidence_quote"),
                "output_segment": link.get("segment"), "source_id": source.get("id"),
                "writer_id": source.get("writer_id"), "trust_level": trust.get("level"),
                "trust_factors": item_trust.get("factors", {}), "policy_status": policy.get("action"),
                "policy_explanation": policy.get("explanation") or (item or {}).get("policy", {}).get("explanation"),
                "policy_reason_codes": policy.get("reason_codes") or (item or {}).get("policy", {}).get("reason_codes", []),
                "influence_score": influence.get("score"), "prompt_included": True, "usage": "used",
            },
        )
        events.append(event)
    ...
    trace = DecisionTrace(
        ...
        metadata={"governed_output_evidence": True, "considered_memories": considered},
    )
    return events, trace
...
def _persist_governed_output(
    gateway: Any, *, tenant_id: str, agent_id: str, session_id: str,
    user_input: str, answer: str, report: dict[str, Any],
) -> None:
    events, trace = governed_output_records(
        tenant_id=tenant_id, agent_id=agent_id, session_id=session_id,
        user_input=user_input, answer=answer, report=report,
    )
    if not events:
        return
    with gateway._lock:
                    ...
                    if governed is not None:
                        ...
                        if fingerprint not in recorded:
                            _persist_governed_output(
                                request.app.state.gateway, tenant_id=principal.tenant_id,
                                agent_id=agent_id, session_id=session_id, user_input=user_input,
